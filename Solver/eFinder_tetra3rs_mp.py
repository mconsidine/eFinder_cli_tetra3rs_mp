...
def solver_process(shm_names, frame_ready, cam_cmd_q, cam_result_q,
    ...
    # drifted across versions (seen: peak_val, peak_value, peak, intensity, mass).
    ...
    try:
        ...
        if len(_probe_extraction.centroids) > 0:
            ...
            for _name in ('peak_val', 'peak_value', 'peak', 'intensity', 'mass'):
                if hasattr(_c, _name):
                    _PEAK_ATTR = _name
                    break
    ...
    def _centroid_peak(centroid_list):
        """Return the brightest centroid's peak intensity as an int, or 0
        if the list is empty or the peak attribute isn't known. Avoids a
        full-frame np.max() scan by reading from the centroid object
        directly — the brightest centroid is always index 0."""
        if not centroid_list or _PEAK_ATTR is None:
            return 0
        try:
    ...
    def _do_solve(img):
        ...
        # Peak brightness comes from the brightest centroid's own attribute
        # (probed at startup); falls back to 0 if no centroids were found
        # or the attribute probe didn't resolve.  This avoids a full-frame
        # np.max() scan (~730K elements) on every frame.
        img_peak = _centroid_peak(centroid_list)
        ...
        # Seeded vs blind solve. tetra3rs has no explicit search-radius
        # parameter yet (tracking mode is on the roadmap), so we approximate:
        #   - on a good prior solve, use a tight timeout (fast path)
        #   - on failure, retry with the full blind timeout
        was_seeded = (solve and solved_radec != (0.0, 0.0))
        kwargs = dict(
            fov_estimate_deg  = fov_est,
            fov_max_error_deg = fov_err,
            image_shape       = (FRAME_H, FRAME_W),
            solve_timeout_ms  = SEEDED_TIMEOUT_MS if was_seeded else BLIND_TIMEOUT_MS,
        )
        sol = db.solve_from_centroids(centroid_list, **kwargs)

        if sol is None and was_seeded:
            print('[solver] seeded solve failed — retrying blind')
            kwargs['solve_timeout_ms'] = BLIND_TIMEOUT_MS
            sol = db.solve_from_centroids(centroid_list, **kwargs)
    ...
    def _handle(cmd, a, b):
        ...
        if cmd == 'adj_exp':
            ...
            for _ in range(20):
                ...
                pk  = _centroid_peak(ext.centroids)
